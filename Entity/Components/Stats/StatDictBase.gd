...
func _init() -> void:
	for modifier_name_index: int in GlobalEnums.POSSIBLE_MODIFIER_NAMES.values():
...
func reset_all_temp_to_default() -> void:
	for modifier_name: int in GlobalEnums.POSSIBLE_MODIFIER_NAMES.values():
		stat_dict[modifier_name].reset_temp_to_default()

func change_modifier_of_given_name( modifier_name: GlobalEnums.POSSIBLE_MODIFIER_NAMES, new_modification: StatModifiers) -> void:
...
func ready_stat_dict() -> void:
	for modifer_name_index: int in GlobalEnums.POSSIBLE_MODIFIER_NAMES.values():
