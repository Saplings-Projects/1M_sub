...
func _on_phase_changed(new_phase: GlobalEnum.Phase, _old_phase: GlobalEnum.Phase) -> void:
	...
	if new_phase == GlobalEnum.Phase.ENEMY_ATTACKING:
...
func _on_player_hand_discarded() -> void:
	PhaseManager.set_phase(GlobalEnum.Phase.ENEMY_ATTACKING)
...
func _on_enemy_start_turn() -> void:
	...
	PhaseManager.set_phase(GlobalEnum.Phase.PLAYER_ATTACKING)
...
func _check_and_handle_battle_end() -> void:
	if PlayerManager.player.get_health_component().current_health == 0:
		PhaseManager.on_combat_end.emit(GlobalEnum.CombatResult.DEFEAT)
	if _enemy_list.is_empty():
		PhaseManager.on_combat_end.emit(GlobalEnum.CombatResult.VICTORY)
