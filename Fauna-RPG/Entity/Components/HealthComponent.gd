...
func deal_damage(damage_data : DealDamageData) -> void:
	...
	var damage_taken_increase = 0.0
	var damage_dealt_increase = 0.0
	...
	var total_damage = damage + damage_taken_increase + damage_dealt_increase

	# apply damage to our health
	var new_health = clampf(current_health - total_damage, 0, max_health)
