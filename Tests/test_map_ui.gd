...
func test_map_initialize() -> void:
	...
	assert_eq(map_ui.room_ui_array[0][2].room.light_data.light_level, GlobalEnums.LightLevel.DIMLY_LIT)
...
func test_movement_and_placing_two_torches() -> void:
	...
	assert_eq(map_ui.room_ui_array[1][1].room.light_data.light_level, GlobalEnums.LightLevel.BRIGHTLY_LIT, "Current room should be brightly lit")
	for room: RoomBase in rooms_in_range:
		if room.room_position.y == 4:
			assert_eq(room.light_data.light_level, GlobalEnums.LightLevel.LIT, "This room was out of range of the previous torch put down, should be lit")
		else:
			assert_eq(room.light_data.light_level, GlobalEnums.LightLevel.BRIGHTLY_LIT, "This room was in range of the previous torch put down, should be brightly lit")
